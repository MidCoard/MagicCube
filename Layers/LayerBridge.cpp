...
void handleInput() {
    ...
    if (glfwGetKey(Render::getWindow()->getWindow(), GLFW_KEY_H) == GLFW_RELEASE && (!StartGameLoop || ifPause)) {
     ...
     if (glfwGetKey(Render::getWindow()->getWindow(), GLFW_KEY_G) == GLFW_PRESS && (!StartGameLoop && !ifPause)) {
        count8++;
        if (count8 == 1) {
            ifDeveloper = !ifDeveloper;
        }
    }
    if (glfwGetKey(Render::getWindow()->getWindow(), GLFW_KEY_G) == GLFW_RELEASE && (!StartGameLoop && !ifPause)) {
        count8 = 0;
    }//开发者信息界面
    ...
    if (!Render::ignoreKeyboardInput && StartGameLoop && !Logic::inSolving && !Logic::inShuffling) {
        ...
        } else if (glfwGetKey(Render::getWindow()->getWindow(), GLFW_KEY_S) == GLFW_PRESS &&
                   !glfwGetKey(Render::getWindow()->getWindow(), GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS) {
            Render::rotate_Y_2(90);
        ...
        if (glfwGetKey(Render::getWindow()->getWindow(), GLFW_KEY_F) == GLFW_PRESS) {
            count4++;
            if (count4 == 1) {
                if (!Logic::inShuffling && !Logic::inSolving) {
                    Logic::solve(true);
        ...
        } else if (glfwGetKey(Render::getWindow()->getWindow(), GLFW_KEY_F) == GLFW_RELEASE) {
            count4 = 0;
        }
        ...
        } else if (glfwGetKey(Render::getWindow()->getWindow(), GLFW_KEY_S) == GLFW_RELEASE &&
                   glfwGetKey(Render::getWindow()->getWindow(), GLFW_KEY_LEFT_CONTROL) == GLFW_RELEASE) {
            count6 = 0;
        }if (glfwGetKey(Render::getWindow()->getWindow(), GLFW_KEY_T) == GLFW_PRESS) {
            count7++;
            if (count7 == 1) {
                ifBackToTitle = true;
            }
        } else if (glfwGetKey(Render::getWindow()->getWindow(), GLFW_KEY_T) == GLFW_RELEASE) {
            count7 = 0;
        }
    }
}